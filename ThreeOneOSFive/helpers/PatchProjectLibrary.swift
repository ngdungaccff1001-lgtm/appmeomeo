...
enum PatchProjectLibrary {
    ...
    static func load(fileManager: FileManager = .default) -> [PatchLibraryItem] {
        ...
        for url in urls where url.pathExtension.lowercased() == "payload" || url.pathExtension.lowercased() == "3105" {
            do {
    ...
    static func save(
        ...
        if let existingURL {
            ...
            var candidate = root.appendingPathComponent(baseName).appendingPathExtension("payload")
            var suffix = 2
            while fileManager.fileExists(atPath: candidate.path) {
                candidate = root.appendingPathComponent("\(baseName)-\(suffix)").appendingPathExtension("payload")
                suffix += 1
            }
